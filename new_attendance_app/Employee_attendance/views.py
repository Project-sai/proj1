...
def ManualAttendanceDisplay(request):
	...
	if request.method == "POST":
		...
		print(args)
		cursor = connection.cursor()
		cursor.callproc('sproc_manualdisplay',args)
		results = cursor.fetchall()
		cursor.close()
		res = results[0][0]
		if res == 'not found':
			mydict["notfound"] = True
		elif res == 'record not found':
			mydict["recnotfound"] = True
		else:
			print(results)
			dates = json.loads(results[0][5])
			date = [i for i in dates.values()]
			total = len(date)
			present = date.count('P')
			absent = date.count('A')
			print(date,total,present,absent)
			mydict = {'name':results[0][1],'empid':results[0][0],'dpt':results[0][2],'designation':results[0][3],'month':results[0][4],
					'total':total,'present':present,'absent':absent,'dates':date,'colspan':total+3}
			print(mydict)
